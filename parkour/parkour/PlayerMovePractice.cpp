...
PlayerMovePractice::PlayerMovePractice(Actor* owner, class GameMode mode): MoveComponent(owner)
{
	ChangeState(PlayerMovePractice::Falling);
	...
	input = PlayerMovePractice::Blank;

	mCurrentMode = mode;
}


PlayerMovePractice::~PlayerMovePractice()
{
...
void PlayerMovePractice::ProcessInput(const Uint8* keyState)
{
	float speed = 0.0f;
	Vector3 forward = Vector3::Zero;
	if(mCurrentState == PlayerMovePractice::Flying)
	{
		speed = Speed_Flying;
	...
	if(keyState[SDL_SCANCODE_K])
	{
		if(!this->mKPressed)
		{
			...
			{
				ChangeState(PlayerMovePractice::Flying);
	...
	if(mCurrentState == PlayerMovePractice::Balance)
	{
	...
	else
	{
		...
		if(keyState[SDL_SCANCODE_W])
		{
			...
			if(mCtrlIsPressing)
			{
				if(input == PlayerMovePractice::W && mCurrentState == PlayerMovePractice::OnGround && !DisableSquat)
				{
			...
			{
				if(input == PlayerMovePractice::W && mCurrentState == PlayerMovePractice::OnGround)
				{
					if(!mShiftPressed)
					{
						mFastRunTimer = 0.0f;
		...
		if(mCurrentState != PlayerMovePractice::Flying)
		{
			...
			{
				if(!mSpacePressed && mCurrentState == PlayerMovePractice::OnGround &&
					      !HeightChange && input != PlayerMovePractice::Shift) 
				{
					Mix_PlayChannel(-mOwner->GetGame()->mJumpSound,
							 mOwner->GetGame()->GetSound("Assets/Sounds/Jump.wav"), 0);
					mSpacePressed = true;
					AddForce(mJumpForce);
					ChangeState(PlayerMovePractice::Jump);
			...
			if(keyState[SDL_SCANCODE_LCTRL])
			{
				...
				if(mWPressed)
				{
					if(input == PlayerMovePractice::Ctrl)
					{
						AddForce(forward * speed);
					}
				}
		...
		if(keyState[SDL_SCANCODE_LSHIFT])
		{
			...
			if(input == PlayerMovePractice::Blank)
			{
				input = PlayerMovePractice::Shift;
			}
...
void PlayerMovePractice::Update(float deltaTime)
{
	touchBottom = false;

	if(mCurrentState == PlayerMovePractice::OnGround)
	{
		UpdateOnGround(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::Jump)
	{
		UpdateJump(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::Falling)
	{
		UpdateFalling(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::WallClimb)
	{
		UpdateWallClimb(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::WallRun)
	{
		UpdateWallRun(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::Flying)
	{
		UpdateFlying(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::Sliding)
	{
		UpdateSliding(deltaTime);
	}
	else if(mCurrentState == PlayerMovePractice::Balance)
	{
...
// Check Sound
	if(
	   ((mCurrentState == PlayerMovePractice::OnGround && mVelocity.Length() >= 50.0f) && !mSquat)||
		mCurrentState == PlayerMovePractice::WallClimb ||
		mCurrentState == PlayerMovePractice::WallRun)
	{
...
void PlayerMovePractice::UpdatePrint(float deltaTime)
{
	
}


void PlayerMovePractice::PhysicsUpdate(float deltaTime)
{
...
void PlayerMovePractice::UpdateSliding(float deltaTime)
{
	...
	if(mSlideTimer > 0.4f)
	{
		ChangeState(PlayerMovePractice::OnGround);
		mSlideTimer = 0.0f;
		HeightChange = true;
		mSliding = false;
	}
...
void PlayerMovePractice::UpdateFlying(float deltaTime)
{
	PhysicsUpdate(deltaTime);
}


void PlayerMovePractice::UpdateOnGround(float deltaTime)
{
	PhysicsUpdate(deltaTime);
	...
	for(auto block: blocks)
	{
		...
		if(side == CollSide::Top)
		{
			...
			if(balance != 0)
			{
				...
				ChangeState(PlayerMovePractice::Balance);
			}
			falling = false;
		}
...
void PlayerMovePractice::UpdateJump(float deltaTime)
{
	AddForce(mGravity);
...
void PlayerMovePractice::UpdateFalling(float deltaTime)
{
	AddForce(mGravity);
	...
	for(auto block: blocks)
	{
		...
		if(side == CollSide::Top)
		{
			...
			if(mSliding)
			{
				ChangeState(PlayerMovePractice::Sliding);
			}
			else
			{
				ChangeState(PlayerMovePractice::OnGround);
...
void PlayerMovePractice::UpdateWallClimb(float deltaTime)
{
...
void PlayerMovePractice::UpdateWallRun(float deltaTime)
{
...
void PlayerMovePractice::UpdateBalance(float deltaTime)
{
	PhysicsUpdate(deltaTime);
...
void PlayerMovePractice::ChangeState(MoveState state)
{
	mCurrentState = state;
}


bool PlayerMovePractice::CanWallRun(CollSide side)
{
...
bool PlayerMovePractice::CanWallClimb(CollSide side)
{
	Vector3 Normal = Vector3::Zero;
...
CollSide PlayerMovePractice::FixCollision(class CollisionComponent* self, class CollisionComponent* block)
{
...
void PlayerMovePractice::AddForce(const Vector3& force)
{
	mPendingForces += force;
}


void PlayerMovePractice::FixXYVelocity()
{
	...
	else if(mCurrentState == PlayerMovePractice::Balance && mUnderBalance)
	{
	...
	if(mCurrentState == PlayerMovePractice::OnGround || 
	   mCurrentState == PlayerMovePractice::WallClimb || 
	   mCurrentState == PlayerMovePractice::Balance)
	{
...
void PlayerMovePractice::FixXYZVelocity()
{
...
void PlayerMovePractice::Respawn()
{
...
void PlayerMovePractice::FixJump()
{
...
Vector3 PlayerMovePractice::FlyingForward()
{
...
void PlayerMovePractice::UpdateSquating(float deltaTime)
{
	if(!touchBottom && !(mCurrentState == PlayerMovePractice::Sliding))
	{
...
void PlayerMovePractice::UpdateFixJump(float deltaTime)
{
	if(mShiftIsPressing)
	{
...
void PlayerMovePractice::UpdateFastRun(float deltaTime)
{
	if(mFastRun)
	{
...
void PlayerMovePractice::SetUpBalance(int balance)
{
	mUnderBalance = true;
...
void PlayerMovePractice::FixBalanceAngle(float deltaTime)
{
